...
- [ ] 13.1 **Write `tests/test_llm_judge.py` FIRST** (TDD):
...
- [ ] 13.3 Add `--llm`, `--llm-backend`, `--llm-model` CLI flags
- [ ] 13.4 Extend `report.py` and `ui.py` for `llm_judge` section + optional `final_ensemble`
- [ ] 13.5 Add `llm_judge` config section to `config.json` and `Config` dataclass
- [ ] 13.6 Integration test: `--online --llm` produces both sections in JSON

## 14. CI / Polish (M8)

- [ ] 14.1 GitHub Actions workflow: `pytest -v`, `ruff check`, `mypy` on Python 3.10, 3.11, 3.12
- [ ] 14.2 Pre-commit hooks: `ruff`, `mypy`
- [ ] 14.3 Verify `pip install -e .` works, `ai-music-checker` runs from PATH
- [ ] 14.4 Update `README.md` with usage examples, config reference, fixture generation
- [ ] 14.5 Run full test suite locally, fix any flaky tests
- [ ] 14.6 Performance check: single file <5s offline, <15s online

## 15. Documentation & Archive

- [ ] 15.1 Final review of PLAN.md vs implemented features
- [ ] 15.2 Update `openspec/changes/ai-music-checker/` artifacts if any drift
